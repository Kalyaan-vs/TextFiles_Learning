...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    year, value = invoice_number.split("-")
...
def record_invoice(invoice_file: TextIO,
    ...
    line in the file. This will be obtained by the previous call
    to `record_invoice`.
    :return: The position of the start of the last line in the file.
    This can be used in subsequent calls to `record_invoices`.
    """
    # last_line_ptr = 0
...
with open(data_file, "r+", newline="") as invoices:
    last_line = record_invoice(invoices, "ACME Roadrunner", 18.40)
    last_line = record_invoice(invoices, "Squirrel Storage", 300.55, last_line)
